  ...
  try {
      ...
      try {
        const info = await transporter.sendMail({
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #b97a7a;">New RSVP Received!</h2>
            <div style="margin: 20px 0; padding: 20px; background: #fdf5f5; border-radius: 8px;">
              <p><strong>Name:</strong> ${name}</p>
              <p><strong>Attendance:</strong> ${attendanceStatus}</p>
              ${guestsSection}
              ${guestNamesSection}
            </div>
            <p style="color: #8e7a7a; font-size: 14px; margin-top: 20px;">
              This RSVP was submitted through the wedding website of Ahmed & Ruaa.
            </p>
          </div>
        `,
        });
    ...
    try {
      info = await transporter.sendMail({
        ...
        html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #b97a7a;">You've received a new message!</h2>
          <p><strong>From:</strong> ${name}</p>
          ${messageType === 'drawn' ? (
            imageFile ? 
              `<p>Here's the handwritten message:</p>
              <div style="margin: 20px 0; padding: 15px; background: #fdf5f5; border-radius: 8px;">
            ...
            textMessage ? 
              ...
              <div style="margin: 20px 0; padding: 15px; background: #fdf5f5; border-radius: 8px; white-space: pre-wrap; line-height: 1.6;">
                ${textMessage.replace(/\n/g, '<br>')}
              </div>` : 
              '<p>No message text was provided.</p>'
          )}
          <p style="color: #8e7a7a; font-size: 14px; margin-top: 20px;">
            This message was submitted through the wedding website of Ahmed & Ruaa.
          </p>
        </div>
      `,
        attachments
